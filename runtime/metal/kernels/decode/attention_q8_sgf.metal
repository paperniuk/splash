...
// Apple7/8 (M1/M2) register-matrix verify attention over INT8 paged KV (the
// Register tile). These GPUs have no bfloat arithmetic, and the MPP tensor
// path spends most of a long-context verify cycle here. Operands:
...
// the shared verify reduce is unchanged.
namespace q8sgf {
using q4sg::Lane;
using q4sg::lane_map;
using q4sg::te;
...
__attribute__((always_inline)) inline void verify_tile(
  ...
  constexpr uint M = SPLASH_TARGET_VERIFY_ROWS * G;
  ...
  static_assert(SPLASH_TARGET_VERIFY_ROWS == 8, "one simdgroup per G fused rows");
